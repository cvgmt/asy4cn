...
还应该弦图的中间是“黄实”，应该用黄色填充。这部分是一个正方形，可以使用现成
...
rotate(`角度`, z=(0,0)) // `\color{comment}按角度绕中心` z`\color{comment}（默认为原点，顺时针）旋转`
...
\begin{enumerate}
  \item 查阅参考手册 \cite{asyman}，看看都有哪些颜色可用。看看你的系统中安装
    了哪些中文字体。然后修改 \autoref{fig:xiantulabel}，使用另一种你喜欢的字
