...
for(int i=0;i<=n;++i){
  ...
  guide[][] px=contour(fx,(a,a),(b,b),new real[]{0},12);
...
for(int i=0;i<=n;++i){
  ...
  guide[][] py=contour(fy,(a,a),(b,b),new real[]{0},12);
...
for(int i=0;i<=n;++i){
  ...
  guide[][] pz=contour(fz,(a,a),(b,b),new real[]{0},12);
  for(int j=0;j<pz[0].length;++j){
    draw(shift(c*Z)*XY()*path3(pz[0][j]),((i>0 && i<n) ? olive : blue+1pt));
  }
}
surface sf=surface(contour3(f,(-5,-5,-5),(5,5,5),12));
